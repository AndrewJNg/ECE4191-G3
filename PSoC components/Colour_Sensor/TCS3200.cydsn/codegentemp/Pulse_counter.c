                                   ...
                                   
{
        ...
        Pulse_counter_GLOBAL_ENABLE &= ((uint8)(~Pulse_counter_BLOCK_EN_MASK));
    #else
        CY_SET_REG32(Pulse_counter_COUNTER_LSB_PTR, counter);
    #endif /* (Pulse_counter_UsingFixedFunction) */
}
#endif /* (!(Pulse_counter_UsingFixedFunction && (CY_PSOC5A))) */
...
uint32 Pulse_counter_ReadCounter(void) 
{
    ...
    #if(Pulse_counter_UsingFixedFunction)
        return ((uint32)CY_GET_REG16(Pulse_counter_STATICCOUNT_LSB_PTR));
    #else
        return (CY_GET_REG32(Pulse_counter_STATICCOUNT_LSB_PTR));
    #endif /* (Pulse_counter_UsingFixedFunction) */
}
...
void Pulse_counter_WritePeriod(uint32 period) 
{
    ...
    #else
        CY_SET_REG32(Pulse_counter_PERIOD_LSB_PTR, period);
    #endif /* (Pulse_counter_UsingFixedFunction) */
}
...
{
    #if(Pulse_counter_UsingFixedFunction)
        return ((uint32)CY_GET_REG16(Pulse_counter_PERIOD_LSB_PTR));
    #else
        return (CY_GET_REG32(Pulse_counter_PERIOD_LSB_PTR));
    #endif /* (Pulse_counter_UsingFixedFunction) */
}
                                   ...
                                   
{
    ...
    #else
        CY_SET_REG32(Pulse_counter_COMPARE_LSB_PTR, compare);
    #endif /* (Pulse_counter_UsingFixedFunction) */
}
...
{
    return (CY_GET_REG32(Pulse_counter_COMPARE_LSB_PTR));
}
